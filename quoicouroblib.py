...
def projection(lat,long, lat_m = 48.199014999999996, long_m = -3.0147983333333332):# Format degrés decimaux
    """
...
def cercle(t,lat_boue,long_boue,k=0,debug=True):
    ...
    T=450 # en s
    N=10
    mx,my = projection(lat_boue,long_boue)
    p_tilde=np.array([mx,my])+r*np.array([np.cos(2*np.pi*((t)/T+k/N)),np.sin(2*np.pi*((t)/T+k/N))])
